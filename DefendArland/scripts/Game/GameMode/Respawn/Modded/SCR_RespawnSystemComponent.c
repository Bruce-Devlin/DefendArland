modded class SCR_RespawnSystemComponent
{
	...
	override void OnPlayerKilled_S(int playerId, IEntity playerEntity, IEntity killerEntity, notnull Instigator killer)
	{
		...
		SetHelpers();
		if (dm != null)
			dm.SetPlayerState(playerId, false);
	}
	...
	protected void ShowIntro1(int secondsToDisplay)
	{
		...
		hint.ShowHint("Prepare to defend.", "Soon russians will try to attack our position, they have us surrounded... Gear up and get ready to fight for our lives.", secondsToDisplay, playerId:dm.localPlayerId, isDebug:dm.hintLogging);
	...
	protected void ShowIntro2(int secondsToDisplay)
	{
		...
		if (dm.endlessMode)
		{
			hint.ShowHint("Survive Forever...", "We have to hold this position for as long as we can!", secondsToDisplay, playerId:dm.localPlayerId, isDebug:dm.hintLogging);
		}
		else
		{
			hint.ShowHint("Survive " + dm.GetNumberOfWaves() + " Waves...", "Backup is arriving soon, we just need to hold them off for a litte while longer!", secondsToDisplay, playerId:dm.localPlayerId, isDebug:dm.hintLogging);
		}
	}
}
